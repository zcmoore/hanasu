...
public class VirtualChannel
{
			...
			int serverPort)
	{
		...
		if (!(serverIPReal.equals("localhost")))
		{
			...
			if (screenManager.createAndConnectServer(serverIPReal, serverPort,
					this.password, screenManager.getUserObject().getNickName()))
			{
		...
		else
		{
			...
			screenManager.getUserObject().setHostName(this.name);
			if (screenManager.createAndConnectServer(severIP, serverPort, this.password,
					screenManager.getUserObject().getNickName()))
			{
				screenManager.getUserObject().setConnected(true);
				return true;
			}
